...
MASK_PATH = Path("F:/Dataset/NFLLONG new normalised/brainmask.nii.gz")
...
def build_datasets(data_root=DATA_ROOT, mask_path=MASK_PATH, seed: int = 0, val_size: int = 10):
    ...
    # 2) Option A: Choose a held-out subset of subjects
    SEED = 0
    random.seed(SEED)
    val_size = 10
    ...
    # 2) Option B: Choose a held-out subset of incomplete modality
    # indexes = random.sample(range(len(sbj_ids)), min(val_size, len(sbj_ids)))
    # heldout_sbjs = {sbj_ids[i] for i in indexes}
    ...
    for sbj_id in sbj_ids:
        ...
        base_path = DATA_ROOT / sbj_id
        try:
            t1 = list(base_path.glob("*_T1.nii"))[0]
            t2f = list(base_path.glob("*_T2.nii"))[0]
            for label, target_glob in enumerate(["*_PBR.nii", "*_PIB.nii", "*_TAU.nii"]):
